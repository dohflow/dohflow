...
require_text "tags are replacement-style" "$ADR" "\`SetTags\` is replacement-style, not a semantic set-union merge"
require_text "tags touched rows choose" "$ADR" "if its write set was touched, it is"
require_text "tags never union-rewrite" "$ADR" "Rebase never rewrites a"
require_text "balance assertions never LWW" "$ADR" "it is never LWW"
require_text "toggle equal state is strict" "$ADR" "Equal end states auto-apply only when the strict predicate holds"
require_text "toggle touched equal state queues" "$ADR" "any touched write set—including one with an equal end state—produces"
...
require_text "rough tally is strict" "$ADR" "23 strict-auto candidate rows (only when the predicate holds)"
require_text "tally forbids touched auto" "$ADR" "must not be read as permission to auto-merge a touched row"
...
require_text "appendix is compared cell by cell" "$ADR" "cell-for-cell against its checked-in output"
require_text "appendix says touched rows queue" "$ADR" "A touched row never"
...
require_text "test parses the ADR table" "$TEST" "fn parse_adr_appendix"
require_text "test compares every ADR cell" "$TEST" "fn compare_adr_appendix"
require_text "test proves one-cell drift fails" "$TEST" "adr_appendix_comparison_rejects_one_cell_outcome_drift"
...
require_text "two-vault toggle strict test" "$TEST" "two_vault_strict_rebase_queues_equal_end_state_toggle_when_touched"
require_text "two-vault tags strict test" "$TEST" "two_vault_strict_rebase_queues_set_tags_without_union_rewrite"
require_text "two-vault untouched auto test" "$TEST" "two_vault_strict_rebase_auto_applies_only_an_untouched_valid_row"
