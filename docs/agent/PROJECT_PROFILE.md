...
- Repository visibility: **this repository (`dohflow/dohflow`) is public**, flipped 2026-09-14 (`personal-cfo-fkt5.9`) after the security-hardening/review bar below was met. Development moved here fully as of `personal-cfo-r36ck` (2026-09-16/17); the original private repository (`chrisbustos/personal-cfo`) will be archived read-only as this bead's final step (after seven days of normal operation and explicit owner approval), never rewritten or deleted in the meantime. A *new* repository this policy would apply to is still private by default until the same bar is met.
