...
| **Settings › connections** | One linked connection, both external accounts mapped, refreshed once, no error. It uses the deterministic mock adapter (accounts-only, so no fixture rows land in the ledger) under the schema token `other`; no real token or network is involved. Because no production adapter carries that id, the app's unlock auto-refresh skips it and the row stays as seeded. Its display hint reads "Mock bridge, 2 accounts" — the one place the fixture shows its seams. |
